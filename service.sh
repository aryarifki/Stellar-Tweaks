...
CPU_GOVERNOR_DEFAULT="interactive"  # Governor default which are desired
CPU_GOVERNOR_FALLBACK="ondemand"    # Governor fallback if interactive is not available
...
set_cpu_governor() {
    ...
    echo "Governor CPU diubah ke: $GOVERNOR"
}

# Function to select default or fallback governor
select_default_governor() {
    AVAILABLE_GOVERNORS=$(get_available_governors)
    if [[ $AVAILABLE_GOVERNORS == *"$CPU_GOVERNOR_DEFAULT"* ]]; then
        echo "$CPU_GOVERNOR_DEFAULT"
    elif [[ $AVAILABLE_GOVERNORS == *"$CPU_GOVERNOR_FALLBACK"* ]]; then
        echo "$CPU_GOVERNOR_FALLBACK"
    else
        echo "schedutil"  # If interactive and ondemand are not available, use schedutil
    fi
}
...
echo "Governor default yang dipilih: $DEFAULT_GOVERNOR"
...
while true; do
    ...
    if $APP_RUNNING; then
        # If there are applications running, set CPU to performance
        set_cpu_governor $CPU_GOVERNOR_PERFORMANCE
       
    else
        # If no applications are running, return the CPU to the default or fallback governor.
